...
/**
 * **Rischio biologico.** Il simbolo internazionale vero: tre lobi uncinati attorno a un disco,
 * dentro un cerchio di contenimento.
 *
 * È l'**emblema della peste** — quello che sta in cima a una schermata, dietro come filigrana, o
 * dentro una pastiglia per dire «questa è roba nostra». Dice appartenenza e pericolo; per il
 * laboratorio c'è {@link MoleculeIcon}, per la malattia {@link VirusIcon}.
 *
 * ⚠️ **Non scende sotto i 32px, ed è una proprietà del disegno, non una preferenza.** I tre lobi
 * si toccano quasi al centro: sotto quella misura si chiudono l'uno sull'altro e il trifoglio
 * diventa una macchia tonda. Dove serve un segno piccolo della peste si usa
 * {@link MoleculeIcon}, che ha i dischi staccati e regge a 16.
 *
 * ⚠️ **Il cerchio di contenimento è il primo a sparire**: è spesso 4 su una griglia da 150, cioè
 * meno di un pixel finché l'icona non passa i 40. È voluto — è il recinto, non il soggetto.
 *
 * ⚠️ E arriva **senza** l'`opacity: 0.6` che ha su `ludoratti.it`: lì è una filigrana di fondo, e
 * quanto debba essere tenue lo decide chi la usa, non il disegno.
 */
export function BiohazardIcon(props: IconProps) {
  return (
    <IconBase {...props} viewBox="-15 -15 150 150">
      <circle cx="60" cy="60" r="70" fill="none" stroke="currentColor" strokeWidth="4" />
      <path d={TREFOIL_LOBES} fillRule="evenodd" clipRule="evenodd" />
      <path d={TREFOIL_RINGS} fillRule="evenodd" clipRule="evenodd" />
    </IconBase>
  );
}
